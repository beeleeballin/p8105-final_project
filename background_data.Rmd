...
in six data sets. The data import and cleaning of the original data can be found 
in the [data import folder](https://github.com/beeleeballin/p8105-final_project/tree/main/data_import)
...
Let's plot the combined longitudinal state-level health outcomes data with 
year on the x-axis, age-adjusted incidence rate on the y-axis, stratified by 
state (color) and outcome (facet). Please note each y-axis has its own scale.
```{r}
...
lc_mel_asthma_state %>%
  ...
  theme_bw() +
  labs(
    title = "Age-Adjusted Incidence Rate for each Health Outcome by State",
    x = "Year",
...
We can define a function to generate maps for a given outcome.
```{r}
...
map_by_outcome(lc_mel_asthma_county, 
               "asthma", 
               "Age-Adjusted Asthma Incidence Rates (2016)")
```
...
map_by_outcome(lc_mel_asthma_county, 
               "melanoma", 
               "Age-Adjusted Melanoma of the skin Incidence Rates (2014-2018)")
```
...
map_by_outcome(lc_mel_asthma_county, 
               "lung cancer", 
               "Age-Adjusted Lung Cancer Incidence Rates (2014-2018)")
```
