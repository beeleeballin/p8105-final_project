...
tidy_pm_df =
  ...
  mutate(
    hover = paste(county, '<br>', 
                  "Seasonal Max", pm25_max_pred, '<br>',
                  "Seasonal Median", pm25_med_pred, '<br>')
  ) %>% 
  select(year, season, countyfips, county, state, PM2.5, hover)
...
tidy_oz_df =
  ...
  mutate(
    hover = paste(county, '<br>', 
                  "Seasonal Max", o3_max_pred, '<br>',
                  "Seasonal Median", o3_med_pred, '<br>')
  ) %>% 
  select(year, season, countyfips, county, state, O3, hover)

tidy_uv_df =
  tidy_df %>% 
  select(-contains("pm25"), -contains("o3")) %>% 
  rename("UV" = edd) %>% 
  mutate(
    hover = paste(county, '<br>', 
                  "Seasonal EDR", edr, '<br>',
                  "Seasonal I305", i305, '<br>',
                  "Seasonal I380", i380, '<br>')
  ) %>% 
  select(year, season, countyfips, county, state, UV, hover)
