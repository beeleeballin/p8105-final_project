...
The data tidying code and details about the useable dataset can be found on the [Climate Data](merge_ap_uv.html) page.
...
Again, a significant amount of cleaning was required. Both the tidied and adjusted state-level and county-level datasets and relevant codes can be found on the [Health Outcomes Data](health_outcomes_data.html) page.
...
After paring down our lung cancer model with BIC, we observed an R^2 value of 0.97, which felt unreasonably high. Diagnostic plots like Q-Q and Box-Cox were conducted, which are viewable on the [Regression](regression.html) page.

With our melanoma BIC model, we observed an adjusted R^2 value of 0.88, which 
also raised some red flags. Diagnostic plots like Q-Q and and residuals vs. 
leverage were conducted, which are also viewable on the [Regression](regression.html) page.
...
With a R^2 value of 0.37, this lung cancer model appears to be far more reasonable
...
Additional visualizations of these models are avalaible. Please visit our [Regression](https://beeleeballin.github.io/p8105-final_project/regression.html) page, or scroll to our **Discussion** for an overview of the findings.
...
the relevant predictors for lung cancer, including age, race, and, perhaps most 
importantly, smoking.
...
was still the most significant in both leave-one-out and Monte Carlo. We, again,
...
longitudinal model, and there are similarly no high outliers.Yet again, the
...
of EDD values in the winter and spring. This was an interesting finding, as you
may expect summer EDD, when it is most likely to be sunny, to have the most effect.
However, this relationship may changed based on sunscreen use. Individuals may be
more likely to use sunscreen in the summer, leaving their skin more vulnerable
in the off-season months. 

Finally, we can observe our asthma cross-sectional model. In the diagnostic plots 
...
residual plot but large abnormality. Again, we see that BIC is the best model through
